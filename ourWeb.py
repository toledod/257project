...
def month(month):
    ...
    cur = conn.cursor()
    sql = "SELECT name, artists, date, pop FROM spotify WHERE EXTRACT(MONTH FROM date) = %s AND country = 'US' ORDER BY pop DESC, date ASC"
...
def day(month, day):
    ...
    return render_template("day2.html", songName = songName, songArtist = songArtist, songDay = songDay)
